...
{{ org.name }} has a legal duty to ensure, so far as is reasonably practicable, the health and safety of its workers and of other people, including participants and visitors, who could be affected by its work. This policy sets out how {{ org.name }} meets that duty under {% for state in org.states %}the {{ whs_act[state | upper] | default('work health and safety legislation of ' ~ state ~ ' [TO CONFIRM]') }}{% if not loop.last %} and {% endif %}{% endfor %} across its {{ intake.homes | length }} home{% if intake.homes | length != 1 %}s{% endif %} and approximately {{ wf.headcount | default('[TO CONFIRM]', true) }} workers. It covers duties, consultation, hazard reporting, manual handling, violence and aggression, working alone and sleepovers, fatigue, psychosocial hazards, and notification of notifiable incidents to the state regulator. It evidences Core Module outcome 4.1 (safe environment) and SIL supplementary module outcome 3 (practice governance and workforce).

## Scope

This policy applies to every worker ({{ wf.employment_types | join(', ') }}), key personnel, contractor, agency worker, student and volunteer of {{ org.name }}, in every home, in the community, in vehicles and when working from other locations. The home environment itself is covered in the Safe Environment and Property Maintenance Policy; infection control in the Waste Management and Infection Control Policy.

## Policy statement

- **Duties.** {{ org.name }} (as the {% if ns.vic %}employer{% else %}person conducting a business or undertaking{% endif %}) provides and maintains a safe working environment, safe systems of work, information, training and supervision, and monitors workers' health and workplace conditions. {{ director }} and other officers exercise due diligence: they keep up to date with WHS matters, understand the hazards of SIL work, ensure resources and processes are in place, and verify that they are used. Every worker takes reasonable care for their own safety and for others, follows reasonable instructions and this policy, and reports hazards and incidents.
- **Consultation.** {{ org.name }} consults workers on WHS matters that affect them: at every house meeting, at the monthly all-staff meeting, before changing rosters, equipment or procedures, and when assessing risk. Workers may elect a health and safety representative, and {{ org.name }} supports the election and training that the legislation provides for.
- **Risk management.** Hazards are identified, risks assessed and controlled using the hierarchy of controls (eliminate first; then substitute, isolate, engineer, administer, and use personal protective equipment last), recorded on the Risk Register and in each home's risk assessment under the Risk Management Policy.
- **Hazard reporting.** Every worker reports every hazard, near miss and injury the same shift in {{ incident_software }}; no report is ever discouraged or penalised.
- **Manual handling.** Hazardous manual tasks (transfers, repositioning, showering, pushing wheelchairs, carrying shopping) are assessed for each participant by an occupational therapist or physiotherapist where needed, with the safe method recorded in the participant's support plan. Workers use hoists, slide sheets and other equipment as trained and never lift a participant's full weight manually. Manual handling training is completed at induction and annually.
- **Violence and aggression.** Behaviours of concern, aggression from any person and occupational violence are psychosocial and physical hazards. Each participant's support plan{% if sup.behaviour_support_plans or sup.restrictive_practices != 'none' %} and behaviour support plan{% endif %} records known triggers, early signs, de-escalation strategies and what to do if a worker is at risk. Workers may withdraw to safety, call the on-call manager or 000, and are never required to use a restrictive practice outside the Restrictive Practices and Behaviour Support Policy. Every assault or threat is an incident, and workers are offered support and an employee assistance service.
- **Working alone{% if ns.sleepover or ns.overnight %} and overnight shifts{% endif %}.** Many SIL shifts are worked by one worker. Every home has an on-call manager available 24 hours, the on-call number is displayed, workers carry a charged phone, and welfare checks are made as the home's risk assessment requires. {% if ns.sleepover %}Sleepover workers ({% for home in intake.homes %}{% if home.roster_model == 'sleepover' %}{{ home.name }}{% if not loop.last %}, {% endif %}{% endif %}{% endfor %}) have a private, lockable room, bedding, bathroom access and a means to summon help, and record every interruption so that the overnight model can be reviewed under the Practice Governance and Workforce Consistency Policy.{% endif %} {% if ns.overnight %}Awake overnight workers ({% for home in intake.homes %}{% if home.roster_model in ['twenty_four_seven', 'active_night'] %}{{ home.name }}{% if not loop.last %}, {% endif %}{% endif %}{% endfor %}) have adequate lighting, a means to summon help and a documented check-in with the on-call manager.{% endif %} {% if ns.dropin %}Drop-in workers ({% for home in intake.homes %}{% if home.roster_model == 'drop_in' %}{{ home.name }}{% if not loop.last %}, {% endif %}{% endif %}{% endfor %}) confirm arrival and departure with the on-call manager or in {{ rostering_software }}.{% endif %}
- **Fatigue.** Rosters in {{ rostering_software }} follow the Practice Governance and Workforce Consistency Policy: no more than 12 hours of active work, at least 10 hours between shifts, sleepovers rostered in line with the Social, Community, Home Care and Disability Services Industry Award 2010, and limits on consecutive days. Workers report fatigue and are not rostered for additional shifts when they do; {{ rostering_manager }} monitors overtime and interrupted sleepovers monthly.
- **Psychosocial hazards.** Workload, exposure to distressing events, bullying, poor support, lone work and remote work are managed as psychosocial hazards under the WHS regulations, with supervision, debriefing after incidents, and the Grievance and Disciplinary Policy.
- **First aid, PPE and emergencies.** {% if wf.first_aid_all %}All workers hold a current first aid certificate.{% else %}At least one worker with a current first aid certificate is rostered on every shift in every home until all workers are certified.{% endif %} Each home has a stocked first aid kit, and the personal protective equipment identified in the home and participant risk assessments; the Emergency and Disaster Management Plan covers fire, medical and other emergencies.
- **Vehicles.** {% if sup.transport %}Workers transporting participants hold a current licence, use vehicles that are registered, insured, serviced and fitted with the restraints the participant needs, do not use a phone while driving, and report every vehicle incident.{% else %}{{ org.name }} does not currently transport participants in vehicles; if this changes, vehicle safety controls are added to this policy before transport begins.{% endif %}
- **Injury and return to work.** Injured workers receive first aid and medical care, workers compensation claims are lodged under the state scheme, and {{ org.name }} supports return to work.
...
1. Make the situation safe if it can be done without risk; give first aid; call 000 for any emergency.
...
5. {{ incident_officer }} and {{ whs_officer }} investigate every injury and every serious near miss under the Incident Management Policy, and the outcome is shared with workers at the next house meeting.

### Part B — Notifiable incidents

1. A notifiable incident is the death of a person, a serious injury or illness of a person (for example one needing immediate treatment as an in-patient, or immediate treatment for amputation, serious head or eye injury, serious burn, spinal injury, loss of a bodily function or serious laceration, or medical treatment within 48 hours of exposure to a substance), or a dangerous incident that exposes a person to a serious risk to health or safety (for example an uncontrolled fire, electric shock, gas leak or structural collapse). It applies to workers, participants and any other person affected by {{ org.name }}'s work.
2. The worker phones the house leader immediately; the house leader phones {{ whs_officer }} and {{ director }}.
3. {{ whs_officer }} notifies the regulator immediately after becoming aware, by phone: {% for state in org.states %}{{ state }} — {{ regulator[state | upper] | default('[TO CONFIRM regulator]') }}{% if not loop.last %}; {% endif %}{% endfor %}, then in writing if the regulator asks{% if ns.vic %} (in Victoria, a written record of the incident is also given to WorkSafe within 48 hours){% endif %}.
4. The site is preserved as far as possible until an inspector arrives or the regulator says it need not be, except to help an injured person, make the area safe or as police direct.
5. {{ whs_officer }} keeps a record of the notification for at least 5 years and files it against the incident in {{ incident_software }}.
...
3. A missed check-in triggers a phone call from the on-call manager within 15 minutes and, if there is no response, a visit or a call to police for a welfare check.

## Records kept

- WHS Risk Register and per-home risk assessments; participant manual handling assessments in support plans.
- Hazard, near miss and injury reports and investigations in {{ incident_software }}.
- Regulator notifications and correspondence; workers compensation and return to work records.
- Consultation records (house meeting and all-staff minutes); health and safety representative election records if any.
...
{% for state in org.states %}- {{ whs_act[state | upper] | default('Work health and safety legislation of ' ~ state ~ ' [TO CONFIRM]') }} — primary duty of care, officers' due diligence, workers' duties, consultation, and incident notification (notifiable incidents, site preservation and records)
